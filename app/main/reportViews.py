...
from ..models import Report,Job,Testsuit
from . import main
...
def view(id):
    ...
    try:
        ...
        suit = Testsuit.query.filter_by(id=job.relatesuitid).first()
    except:
    ...
    if report is not None:
        ...
        reportDetail['测试服务'] = job.address
        reportDetail['服务版本'] = job.serviceversion
        reportDetail['测试时间'] = job.createdtime
        reportDetail['日志路径'] = report.logpath
        reportDetail['测试集and大小'] = "{name} | {size}".format(name=suit.name,size=suit.size)
        ...
        reportDetail['最小and最大响应时间'] = "{min}~{max}秒".format(min=report.min_span,max=report.max_span)
